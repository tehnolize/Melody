...
- Перенесены кнопки загрузки `+ mp3` и `+ папка` в блок **Альбомы** для более понятного UX.
- Добавлена валидация: загрузка треков недоступна, пока не создан хотя бы один альбом.
- Блок альбомов переделан в **список** (вместо выпадающего списка): можно открыть, переименовать, удалить.
- При открытии альбома очередь проигрывателя заполняется именно треками выбранного альбома.
- Для кнопки **В мой альбом** добавлен диалог выбора конкретного альбома.
- Чат хранится по пользователям отдельно (ключи localStorage вида `mw_chat_<userId>`).
- При выходе показывается чистый экран входа без данных предыдущего пользователя.
