...
/**
 * Создаёт маршрутизатор для аутентификации.
 * @param {import('pg').Pool} pool - Пул подключений к PostgreSQL.
 * @param {string} uploadsRoot - Корневая директория для загрузок.
 * @param {{ info: Function, success: Function, error: Function }} log - Объект для логирования.
...
export function createAuthRouter(pool, uploadsRoot, log) {
  ...
  router.patch('/api/me/location', requireAuth, async (req, res) => {
    try {
      ...
      await pool.query(
        `UPDATE users SET lng = $1, lat = $2 WHERE id = $3`,
        [lng, lat, res.locals.userId]
      );
      res.json({ ok: true });
    } catch (e) {
