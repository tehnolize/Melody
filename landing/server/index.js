...
if (!process.env.JWT_SECRET || process.env.JWT_SECRET.length < 16) {
  log.error('JWT_SECRET is not set or too short (min 16 characters).');
  process.exit(1);
}
...
const openai = OPENAI_API_KEY && OPENAI_API_KEY.length > 10
...
if (openai) {
  log.success('OpenAI API configured', {
    keyLength: OPENAI_API_KEY.length,
    keyPrefix: OPENAI_API_KEY.substring(0, 7) + '...'
  });
} else {
  log.warn(
    'OpenAI API key not found or invalid – GPT chat will be disabled.',
    { hasKey: !!process.env.OPENAI_API_KEY, keyLength: OPENAI_API_KEY?.length || 0 }
  );
}

// ----- Вспомогательные функции для статистики GPT -----
function formatUsageStats() {
  const remaining = Math.max(0, INITIAL_BALANCE - totalSpent);
  const estimatedRequestsLeft = remaining > 0
    ? Math.floor(remaining / (GPT_INPUT_PRICE * 200 + GPT_OUTPUT_PRICE * 150))
    : 0;
  ...
  return {
    ...
    estimatedRequestsLeft: estimatedRequestsLeft,
  };
}
...
function logUsageBox(stats, requestCost, inputTokens, outputTokens, totalTokens) {
  ...
  const usageLines = [
    ...
    `Input Tokens:       \x1b[36m${String(inputTokens).padStart(6)}\x1b[0m`,
    `Output Tokens:      \x1b[36m${String(outputTokens).padStart(6)}\x1b[0m`,
    `Total Tokens:       \x1b[36m${String(totalTokens).padStart(6)}\x1b[0m`,
...
// Добавление pool и uploadsRoot в каждый запрос
...
// Роуты (импортируемые из других файлов)
// Ниже приведены маршруты, описанные в вашем коде; предполагаем, что они определены в отдельных модулях
// и подключаются как:
...
// ----- Эндпоинты, которые остаются в этом файле (зависит от логики) -----
app.post('/api/chat', async (req, res) => {
  const msg = String(req.body?.message || '').trim();
  if (!msg) {
    return res.json({ reply: 'Напиши сообщение' });
  }
...
app.get('/api/popular', async (req, res) => {
  ...
  try {
    const bb = await fetch('https://www.billboard.com/charts/hot-100/', { headers: headers, redirect: 'follow' });
    if (bb.ok) {
      const html = await bb.text();
      const items = parseBillboardBasic(html, limit);
      if (items.length > 0) {
        popularCache = { t: now, items: items };
  ...
  try {
    const ac = await fetch('https://acharts.co/us_singles_top_100', { headers: headers, redirect: 'follow' });
