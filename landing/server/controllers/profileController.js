...
export async function getMyProfile(req, res) {
  try {
    ...
    const tq = await req.pool.query(
      `SELECT t.id, t.title, t.storage_name, t.original_name,
              t.file_size, t.created_at,
              COALESCE(
 ...
 *
 * @param {Object} req - Express request object (includes req.pool, req.params).
 * @param {Object} res - Express response object.
 * @returns {Promise<void>}
 */
export async function getPublicProfile(req, res) {
  try {
    ...
    const tq = await req.pool.query(
      `SELECT id, title, storage_name, created_at
       FROM tracks
       WHERE user_id = $1
